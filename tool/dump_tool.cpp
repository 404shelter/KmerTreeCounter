...
inline char* format_uint32(uint32_t n, char* buf_end) {
    do { *--buf_end = '0' + (n % 10); n /= 10; } while (n);
    return buf_end;
}
...
public:
    void open(const std::string& path) {
        ...
        if (fd_ < 0) { std::cerr << "open failed: " << path << '\n'; exit(1); }
        for (int i = 0; i < NUM_BLOCKS; ++i) free_list_.push_back(new char[BLOCK_SIZE]);
    ...
    void finish() {
        ...
        thread_.join();
        ::close(fd_);
        for (char* buf : free_list_) delete[] buf;
        while (!ready_queue_.empty()) { delete[] ready_queue_.front().first; ready_queue_.pop(); }
    }
...
template <typename KeyT> struct SortEntry { KeyT key; uint32_t count; };
...
static void parallel_radix_sort(std::vector<SortEntry<KeyT>>& entries,
    ...
    if (src == temp.data()) entries.swap(temp);
}
...
static Options parse_options(int argc, char* argv[]) {
    ...
    if (std::strcmp(argv[1], "precise") == 0)       opts.is_precise = true;
    else if (std::strcmp(argv[1], "approximate") == 0) opts.is_precise = false;
    ...
    if (opts.k_len == 0 || opts.k_len > MAX_K) { std::cerr << "invalid k_len\n"; exit(-1); }
    if (opts.max_threads == 0)                 { std::cerr << "invalid max_threads\n"; exit(-1); }
    if (opts.max_freq < opts.min_freq)         { std::cerr << "max_freq < min_freq\n"; exit(-1); }
    return opts;
}
...
static std::vector<RootFileInfo> collect_root_files(const std::string& tmp_dir,
    ...
    for (uint64_t id = 0; id < ROOT_BUCKET_COUNT; ++id) {
        ...
        if (records > 0) files.push_back({std::move(path), records, size});
    }
    return files;
}
...
static void build_hash_map(const std::vector<RootFileInfo>& files,
    ...
    auto worker = [&]() {
        ...
        for (;;) {
            ...
            if (fd < 0) { std::cerr << "open: " << info.filename << '\n'; exit(-1); }
            ...
            while (remaining > 0) {
                ...
                if (n != static_cast<ssize_t>(bytes)) { std::cerr << "short read\n"; exit(-1); }
...
static void process_low_freq(const std::string&              low_path,
    ...
    ::close(fd);
    if (mapped == MAP_FAILED) { std::cerr << "mmap low.bin failed\n"; exit(-1); }
...
static void process_low_freq_collect(const std::string&              low_path,
    ...
    for (auto& v : thread_results) v.reserve(total_kmers / worker_count + 1024);
...
static int run_precise(const Options& opts) {
    ...
    uint64_t low_bytes = 0;
    {
        ...
        if (low_bytes % pkb != 0) { std::cerr << "low.bin size error\n"; exit(-1); }
    }
...
static int run_approximate(const Options& opts) {
    ...
    if (opts.sort_output) {
        ...
        for (auto& v : thread_results) v.reserve(total_records / wc + 1024);
        std::atomic<uint64_t> next_file{0};
        ...
        auto worker = [&](uint32_t tid) {
            ...
            for (;;) {
                uint64_t index = next_file.fetch_add(1, std::memory_order_relaxed);
                if (index >= root_files.size()) break;
        ...
        std::vector<std::thread> workers;
        workers.reserve(wc);
        for (uint32_t i = 0; i < wc; ++i) workers.emplace_back(worker, i);
        for (auto& w : workers) w.join();
        progress.finish();
        ...
        std::vector<std::thread> workers;
        workers.reserve(wc);
        for (uint32_t i = 0; i < wc; ++i) workers.emplace_back(worker);
        for (auto& w : workers) w.join();
        writer.finish();
        progress.finish();
    }
